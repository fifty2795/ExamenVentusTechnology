...
function validarRangoFechasYFormatear(idInicio, idFinal) {
    ...
    if (!fechaInicio) {
        mostrarAlerta(idInicio, 'Debe completar fecha inicio', 'warning');
        return false;
    } else if (!fechaFin) {
        mostrarAlerta(idFinal, 'Debe completar fecha fin', 'warning');
        return false;
    }

    // Validar formato de ambas fechas - Formato: 'dd/mm/yyyy'
    if (!validarFecha(fechaInicio)) {
        mostrarAlerta(idInicio, 'Ingresa una fecha inicio con el formato dd/mm/yyyy', 'warning');
        return false;
    } else if (!validarFecha(fechaFin)) {
        mostrarAlerta(idFinal, 'Ingresa una fecha fin con el formato dd/mm/yyyy', 'warning');
        return false;
    }
    ...
    if (inicio > fin) {
        mostrarAlerta(idInicio, 'La fecha de inicio no puede ser mayor que la fecha final', 'warning');
        return false;
    }
